...
echo "  ./scripts/swarm-quickstart.sh        # Premier run : check → sd init → 2 issues test → pipeline (ou --yes)"
echo "  sd init   # si tu n'utilises pas quickstart (déjà fait par quickstart)"
...
echo "  ./scripts/swarm-prompt.sh \"Ta demande\"   # Entrée langage naturel (LITELLM_API_BASE ou OPENROUTER_API_KEY requis)"
